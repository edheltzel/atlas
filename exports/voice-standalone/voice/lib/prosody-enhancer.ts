 ...
 */
export function detectEmotionalContext(message: string): string | null {
  if (/\[(💥|✨|⚠️|🚨|🎉|💡|🤔|🔍|📈|🎯|🎨|🐛|📚)/.test(message)) {
    return null;
  }
 ...
 */
export function getEmotionalMarker(emotion: string): string {
 ...
 */
export function addPersonalityProsody(message: string, personality: AgentPersonality): string {
...
export function cleanForSpeech(message: string): string {
  ...
  while ((match = markerRegex.exec(cleaned)) !== null) {
    if (match.index > lastIndex) {
      parts.push({ isMarker: false, text: cleaned.substring(lastIndex, match.index) });
    }
    parts.push({ isMarker: true, text: match[0] });
    lastIndex = match.index + match[0].length;
  }

  if (lastIndex < cleaned.length) {
    parts.push({ isMarker: false, text: cleaned.substring(lastIndex) });
  }

  if (parts.length === 0) {
    parts.push({ isMarker: false, text: cleaned });
  }

  cleaned = parts.map(part => {
    if (part.isMarker) return part.text;
...
export function getAgentPersonality(agentType: string): AgentPersonality {
  return AGENT_PERSONALITIES[agentType.toLowerCase()] || AGENT_PERSONALITIES['atlas'];
}
