 ...
 * Reads timezone from settings.json via principal.timezone or env.TIME_ZONE
 * Used by: All hooks that need timestamps
 *
 * Ported from PAI v2.3.0
 */

import { getPrincipal } from './identity';

/**
 * Get configured timezone from settings.json (defaults to America/New_York)
 */
function getTimezone(): string {
  return getPrincipal().timezone || 'America/New_York';
}
